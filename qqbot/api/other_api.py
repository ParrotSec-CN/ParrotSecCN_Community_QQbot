...
# query_free_v2ray_node
def v2_node():
    v2_link = "https://raw.githubusercontent.com/ssrsub/ssr/master/v2ray"

    v2_result = requests.get(v2_link)
    share_links = base64.b64decode(v2_result.content).decode('utf-8').splitlines()
    
    choice_node = random.choice(share_links) 
    v2_info = "当前v2订阅地址：{}{}{}{}".format(v2_link, '\n', choice_node, '\n')
    return v2_info

# query_free_ssr_node
def ssr_node():
    ssr_link = "https://raw.githubusercontent.com/ssrsub/ssr/master/ssrsub"

    ssr_result = requests.get(ssr_link)
    share_links = base64.b64decode(ssr_result.content).decode('utf-8').splitlines()
    
    choice_node = random.choice(share_links) 
    ssr_info = "当前ssr订阅地址：{}{}{}{}".format(ssr_link, '\n', choice_node, '\n')
    return ssr_info

def get_server_link():
    all_node_info = "{}{}".format(v2_node(), ssr_node())
