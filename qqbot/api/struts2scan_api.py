...
class S2_001:
    ...
    def __init__(self, url, data=None, headers=None, encoding="UTF-8"):
        ...
        if not data:
            ...
            self.data = data
        dict_header = dict()
        dict_header['User-Agent'] = choice(random_headers)
...
def scan_one(url, encoding="UTF-8"):
    reult_info = ""
    """扫描单个URL漏洞"""
    click.secho('[+] 正在扫描URL:' + url, fg='green')
    ss = [s(url, encoding) for s in s2_list]
    ...
    for r in results:
        if r.startswith("ERROR:"):
            ...
            reult_info += '[ERROR] {url} 访问出错: {error}'.format(url=url, error=r[6:]) + '\n'
            return reult_info
        else:
            click.secho('[*] {url} 存在漏洞: {name}'.format(url=url, name=r), fg='red')
            reult_info += '[*] {url} 存在漏洞: {name}'.format(url=url, name=r) + '\n'
            return reult_info
...
def struts2_scan_function(url, proxy=None):
    """Struts2批量扫描利用工具"""
    global proxies
    if proxy:
        proxies = {
    ...
    if url:
        reult_infos = scan_one(url, encode)
        return reult_infos


# if __name__ == '__main__':
#     url = "http://192.168.154.139:8899"
#     try:
#         struts2_scan(url)
